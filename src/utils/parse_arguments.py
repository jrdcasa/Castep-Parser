...
def parse_arguments(program):
    ...
    if program.upper() == "CASTEP":
        ...
        group.add_argument("-d", "--dir", dest="dircp2k", nargs="+",
                           help="A list of directories to search for *.castep output files from CP2K.",
                           action="store", default=None)
    ...
    if program.upper() == "CASTEP":
        ...
        if args.dircp2k is not None:
            for item in args.dircp2k:
