...
from src.castep_output import CastepOutput
...
def write_summary(energy_dict_sorted, cluster_dict, cluster_centroid_lowest, logger=None):
    ...
    # Write table
    m = "\t\t{0:1s} {1:^35s} {2:^19s} {3:^8s} {4:^s}\n".format("#", "Name_job", "Energy(ev)",
    ...
    for idx, item in enumerate(energy_dict_sorted.items()):
        ...
        if idx == 0:
            e_min = energy
        try:
            line = "\t\t{0:^3d} {1:^42s} {2:10.5f} {3:6.2f} {4:^6d}\n".format(idx,
                                                                              key,
                                                                              energy,
                                                                              (energy-e_min)*ev_to_kcalmol,
                                                                              cluster_dict[key])
        except (Exception,):
            line = "\t\t{0:^3d} {1:^42s} {2:10.5f} {3:6.2f} {4:s}\n".format(idx,
                                                                            key,
                                                                            energy,
                                                                            (energy-e_min)*ev_to_kcalmol,
                                                                            "Not clustering")
...
def write_tcl_pdbs():
    ...
    lines += 'set rep 0\n'
    lines += 'newRep "all" "DynamicBonds 1.900000 0.100000 12.000000" "name" $rep $imol1 \n'
    lines += 'set rep 1\n'
...
def write_gnuplot_templates(energydict, isconvergeddict):
    ...
    lines += '\n'
    lines += 'filelist = system("ls ./02-ITER_DAT/*.dat")\n'
    ...
    with open("./02-ITER_DAT/Energy.dat", 'w') as fvmd:
        ...
        lines = '#Structure Label Energy(eV) DE(kcal/mol) 1/0_Not_converged/COnverged\n'
...
def main_app():
    ...
    versionapp = 1.0
    # Parse arguments
    args = src.utils.parse_arguments.parse_arguments("CASTEP")
    # Setup log
    ...
    nfiles = 0
    if args.filecastep is not None:
        c = CastepOutput(args.filecastep, args.dircastep, unwrap_coordinates=args.unwrap, logger=logger)
        nfiles = 1
        key = os.path.splitext(os.path.split(args.filecastep)[-1])[0]
        energy_dict[key] = c.getoptenergy()
        converged_dict[key] = c._issuccessopt
    elif args.dircastep is not None:
        files = src.utils.find_files_recursive.find_files_matching_pattern(args.dircastep, "*.castep")
        # Create the objects
        for ifile in files:
            c = CastepOutput(ifile, args.dircastep, unwrap_coordinates=args.unwrap, logger=logger)
            # print(ifile.replace(args.dircastep, ''))
            # key = os.path.splitext(os.path.split(ifile)[-1])[0]
            key = ifile.replace(args.dircastep, '')
            if key[0] == "/":
                key = key[1:]
            key = key.replace("/", ".")
            ...
            if not c._issuccessopt:
                converged_dict[key] = c._issuccessopt
                try:
                    energy_even_not_converged_dict[key] = c._iteration_energy_ev[-1]
                except IndexError:
