...
def write_summary(energy_dict_sorted, cluster_dict, cluster_centroid_lowest, logger=None):
    ...
    for idx, item in enumerate(energy_dict_sorted.items()):
        ...
        if idx == 0:
            e_min = energy
        try:
            line = "\t\t{0:^3d} {1:^42s} {2:10.8f} {3:6.2f} {4:^6d}\n".format(idx,
                                                                              key,
                                                                              energy,
                                                                              (energy-e_min)*hartrees_to_kcalmol,
                                                                              cluster_dict[key])
        except (Exception,):
            line = "\t\t{0:^3d} {1:^42s} {2:10.8f} {3:6.2f} {4:s}\n".format(idx,
                                                                            key,
                                                                            energy,
                                                                            (energy-e_min)*hartrees_to_kcalmol,
                                                                            "Not clustering")
...
def main_app():
    ...
    if args.filecp2k is not None:
        ...
        # Create the objects
        for ifile in files:
            c = CP2KOutput(ifile, args.dircp2k, unwrap_coordinates=args.unwrap, logger=logger)
            #print(ifile.replace(args.dircastep, ''))
            #key = os.path.splitext(os.path.split(ifile)[-1])[0]
            key = ifile.replace(args.dircp2k, '')
